...
fun run exp =
  let
    ...
    handle SymbolNotFound => let val p = print ("ERR: symbol not found.") in raise SymbolNotFound end
    ...
    handle SymbolNotFound => let val p = print ("ERR: symbol not found.") in raise SymbolNotFound end
         | HDEmptySeq => let val p =  print ("ERR: can't access head of empty sequence.") in raise HDEmptySeq end
         | TLEmptySeq => let val p =  print ("ERR: can't access tail of empty sequence.") in raise TLEmptySeq end
         | ValueNotFoundInMatch => let val p =  print ("ERR: no match results.") in raise ValueNotFoundInMatch end
         | NotAFunc => let val p =  print ("ERR: not a function.") in raise NotAFunc end
         | Impossible => let val p = print ("ERR: unknown error.") in raise Impossible end
         | _ => let val p = print ("ERR: unknown error.") in raise Impossible end
  in
