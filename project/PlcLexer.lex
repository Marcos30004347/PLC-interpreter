...
"not"     => (Tokens.NOT(yypos, yypos + size yytext));
...
"true"    => (Tokens.TRUE(yypos, yypos+size yytext));
"false"   => (Tokens.FALSE(yypos, yypos + size yytext));


{digit}+  => (Tokens.NUM(revfold (fn (a,r) => ord(a)-ord(#"0")+10*r) (explode yytext) 0, !pos,!pos));
{alpha}+{digit}*   => (Tokens.ID(yytext, yypos, yypos + size yytext));